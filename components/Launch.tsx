...
const Launch = ({ launch }: { launch: ILaunch }) => {
	...
	const formatDate = (launcgesDate: string): string => {
	...
	const checkLaunchStatus = (
		...
		if (upcoming) {
			return `Planning on ${formatDate(date)}`;
		} else {
	...
	return (
			<LaunchWrapper>
				...
				<TextContent>
					<Name>{name}</Name>
					<LaunchDate>Launch date: {formatDate(date)}</LaunchDate>
					<Status success={success} upcoming={upcoming}>
						{checkLaunchStatus(success, upcoming, date)}
					</Status>
					<Description>
						{details || "There is no information yet."}
					</Description>
				</TextContent>
			</LaunchWrapper>
	);
...
const LaunchWrapper = styled.li`
	...
	:hover {
		...
		${PatchImage} {
			...
			transform: scale(1.65) translate(0, 25%);
			opacity: 0.7;
		}
	}
`;
...
const Status = styled.span<TStatus>`
	...
	color: ${(props) =>
		props.upcoming ? "#308aff" : props.success ? "#1db262" : "#d31647"};
	border: ${(props) =>
			props.upcoming ? "#308aff" : props.success ? "#1db262" : "#d31647"}
		2px solid;
`;
